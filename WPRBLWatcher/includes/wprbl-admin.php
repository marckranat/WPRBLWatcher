...
class WPRBL_Admin {
    ...
    public function __construct() {
        ...
        add_action('admin_init', [$this, 'handle_log_download'], 1);
    }
    ...
    public function render_dashboard() {
        ...
        // Ensure database columns exist (for existing installations)
        $this->ensure_preferences_columns_exist();
        ...
        if (!$prefs) {
            ...
            // Ensure debug_logging and log_filename exist and are properly typed
            if (!isset($prefs['debug_logging'])) {
                $prefs['debug_logging'] = 0;
            } else {
                // Cast to integer (MySQL may return as string)
                $prefs['debug_logging'] = (int) $prefs['debug_logging'];
            }
            if (!isset($prefs['log_filename'])) {
                $prefs['log_filename'] = null;
            }
            // Cast email_notifications to integer as well
            if (isset($prefs['email_notifications'])) {
                $prefs['email_notifications'] = (int) $prefs['email_notifications'];
            }
        }
        ...
        // Show debug notice if logging is enabled via preferences
        if (!empty($prefs['debug_logging']) && !empty($prefs['log_filename'])) {
            $log_url = admin_url('tools.php?page=wprbl-monitor&wprbl_download_log=1&_wpnonce=' . wp_create_nonce('wprbl_download_log'));
            add_settings_error('wprbl', 'debug_info', 
                'Debug logging is enabled. <a href="' . esc_url($log_url) . '">Download log file</a>', 
                'info');
        }
    ...
    private function update_preferences($user_id) {
        ...
        // Get existing preferences to preserve log_filename if logging is being enabled
        $existing = $wpdb->get_row($wpdb->prepare("SELECT * FROM $table_prefs WHERE user_id = %d", $user_id), ARRAY_A);
        $log_filename = $existing['log_filename'] ?? null;
        
        // Generate random log filename if enabling logging and one doesn't exist
        if ($debug_logging && empty($log_filename)) {
            $log_filename = 'wprbl-' . bin2hex(random_bytes(16)) . '.txt';
        }
        
        // If disabling logging, optionally clean up the log file
        if (!$debug_logging && !empty($log_filename)) {
        ...
        if ($result === false) {
            add_settings_error('wprbl', 'error', 'Failed to save preferences: ' . $wpdb->last_error, 'error');
        } else {
    ...
    public function handle_log_download() {
        if (!isset($_GET['wprbl_download_log']) || !is_user_logged_in()) {
            return;
        }
        
        check_admin_referer('wprbl_download_log');
        
        global $wpdb;
        $user_id = get_current_user_id();
        $table_prefs = $wpdb->prefix . 'wprbl_user_preferences';
        
        $prefs = $wpdb->get_row($wpdb->prepare("SELECT log_filename FROM $table_prefs WHERE user_id = %d AND debug_logging = 1", $user_id), ARRAY_A);
        
        if (!$prefs || empty($prefs['log_filename'])) {
            wp_die('Log file not found or logging not enabled.');
        }
        
        $log_dir = defined('WP_CONTENT_DIR') ? WP_CONTENT_DIR : ABSPATH . 'wp-content';
        $log_file = $log_dir . '/' . $prefs['log_filename'];
        ...
        if (!$real_log_file || strpos($real_log_file, $real_log_dir) !== 0) {
            wp_die('Invalid log file path.');
        }
        
        header('Content-Type: text/plain');
        header('Content-Disposition: attachment; filename="' . basename($prefs['log_filename']) . '"');
    ...
    private function ensure_preferences_columns_exist() {
        global $wpdb;
        $table_prefs = $wpdb->prefix . 'wprbl_user_preferences';
        
        // Check if from_email column exists
        $from_email_exists = $wpdb->get_var($wpdb->prepare("
            SELECT COUNT(*) 
        ...
        if ($debug_logging_exists == 0) {
            // Add after from_email if it exists, otherwise after email_notifications
            $after_column = $from_email_exists > 0 ? 'from_email' : 'email_notifications';
            $wpdb->query("ALTER TABLE $table_prefs ADD COLUMN debug_logging tinyint(1) DEFAULT 0 AFTER $after_column");
        }
        
        // Check if log_filename column exists
