    ...
    <div class="wprbl-section">
        ...
        <form method="POST">
            ...
            <table class="form-table">
                ...
                <tr>
                    <th><label for="debug_logging">Debug Logging:</label></th>
                    <td>
                        <label>
                            <input type="checkbox" name="debug_logging" value="1" id="debug_logging" <?php checked($prefs['debug_logging'] ?? 0, 1); ?>>
                            Enable debug logging for RBL checks
                        </label>
                        <?php if (!empty($prefs['debug_logging']) && !empty($prefs['log_filename'])): ?>
                            <?php 
                            $log_url = admin_url('tools.php?page=wprbl-monitor&wprbl_download_log=1&_wpnonce=' . wp_create_nonce('wprbl_download_log'));
                            ?>
                            <p class="description">
                                <a href="<?php echo esc_url($log_url); ?>" target="_blank">View log file</a> (<?php echo esc_html($prefs['log_filename']); ?>)
                            </p>
                        <?php else: ?>
                            <p class="description">When enabled, detailed debug information will be logged to a randomly named file.</p>
                        <?php endif; ?>
                    </td>
                </tr>
            </table>
